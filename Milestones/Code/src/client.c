...
int client_auth()
{
    ...
    if(status != 0)
    {
        printf("Authorization failed with username '%s' and password '%s'. " \
...
int client_get(struct storage_record *r)
{
    ...
    if(status != 0)
        ...
        printf("Value returned for key '%s' is '%s'.\n", key, r->value);
...
int client_set(struct storage_record *r)
{
    ...
    if(status != 0)
        printf("Set failed. Error code: %d.\n", errno);
    else
        printf("Value changed for key '%s' in table '%s' to '%s'.\n", key, table, r->value);
...
int client_query()
{
    ...
    {
        printf("Query results: %d key(s) matched with '%s' in table '%s'\n", num_matched_keys, predicates, table);
...
int client_disconnect()
{
    ...
    if(status != 0)
        printf("Disconnection failed. Error code: %d.\n", errno);
    else
    {
