...
START_TEST (test_conn_connectnoserver)
{
	sleep(1);       // Give the OS enough time to kill previous process

	void *conn = storage_connect(SERVERHOST, server_port);
	fail_unless(conn == NULL, "Connecting to non-existent server should fail.");
}
END_TEST
...
/**
 * @brief This runs the marking tests for Assignment 2.
 */
int main(int argc, char *argv[])
{
	...
	tcase_add_test(tc, test_conn_connectnoserver);
	suite_add_tcase(s, tc);
	...
	tcase_add_test(tc, test_getinvalid_invalidtable);
	suite_add_tcase(s, tc);
	...
	// Two server instances tests
	tc = tcase_create("restartserver");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_test(tc, test_restartserver_onetable);
	suite_add_tcase(s, tc);
